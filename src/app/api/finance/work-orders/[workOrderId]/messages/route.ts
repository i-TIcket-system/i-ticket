...
/**
 * GET - Fetch messages for a work order (finance staff access)
 */
export async function GET(
  ...
  try {
    ...
    console.error("Finance messages fetch error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}

/**
 * POST - Send a message on a work order (finance staff can participate)
 */
  ...
  try {
    ...
    const newMessage = await prisma.workOrderMessage.create({
      data: {
        ...
        senderName: session.user.name || "Finance",
        senderRole: "FINANCE",
        message,
        type,
      },
    ...
    console.error("Finance message send error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
