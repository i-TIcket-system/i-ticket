...
async function handleSuccessfulPayment(payment: any, callback: TelebirrCallbackPayload) {
  ...
  await transactionWithTimeout(async (tx) => {
    ...
    // 5. INTERNAL: Create sales commission if user was referred (with 70/30 split)
      ...
      if (hasRecruiter) {
        ...
        await tx.salesCommission.create({
          data: {
            ...
            recruiterCommissionAmount: recruiterShare, // Track how much went to recruiter
        ...
        await tx.salesCommission.create({
          data: {
            ...
            recruiterCommissionAmount: 0, // Recruiter doesn't split further (1 level only)
            recruiterId: null,
            isRecruiterCommission: true, // Flag as recruiter's 30% commission
            status: 'PENDING',
          }
        });
      } else {
        // No recruiter: Sales person gets 100% (Tier 1 admin-created sales persons)
        await tx.salesCommission.create({
          data: {
            salesPersonId: salesReferral.salesPersonId,
            bookingId: payment.booking.id,
