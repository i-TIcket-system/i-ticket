...
/**
 * GET /api/admin/audit-logs/download
 * Super Admin - Download all audit logs as CSV with date range filtering
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const includeNull = searchParams.get('includeNull') === 'true' // Include platform logs (companyId=NULL)
    ...
    if (companyId && companyId !== 'all') {
      ...
      // If no specific company selected and not including NULL, show all company logs
      where.companyId = { not: null }
    }
    // If includeNull is true, don't filter by companyId at all (show everything)
    ...
    const logs = await prisma.adminLog.findMany({
      ...
      take: 50000, // Higher limit for Super Admin
    })
    ...
    for (const log of logs) {
      ...
      const company = log.companyId
        ? companyMap.get(log.companyId)?.name || 'Unknown Company'
        : 'Platform Action'
    ...
    const filename = `i-Ticket_Platform_audit_logs_${dateStr}.csv`
