...
/**
 * BUG FIX v2.10.5: Staff Work Orders Page
 * Read-only view of work orders for Driver/Conductor
 */
...
const STATUS_OPTIONS = [
  { value: "OPEN", label: "Open", color: "bg-blue-600 text-white", icon: Clock },
  { value: "IN_PROGRESS", label: "In Progress", color: "bg-yellow-500 text-gray-900", icon: Play },
  { value: "BLOCKED", label: "Blocked", color: "bg-red-600 text-white", icon: Pause },
  { value: "COMPLETED", label: "Completed", color: "bg-green-600 text-white", icon: CheckCircle },
]
...
const PRIORITY_INFO: Record<number, { label: string; color: string }> = {
  ...
  4: { label: "Urgent", color: "bg-red-600 text-white animate-pulse" },
}
...
export default function StaffWorkOrdersPage() {
  ...
  const fetchWorkOrders = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/staff/work-orders?${params}`)
  ...
  const roleLabel = session?.user?.staffRole === "DRIVER" ? "Driver" : "Conductor"
    ...
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="p-3 rounded-xl bg-gradient-to-br from-teal-100 to-teal-200 border border-teal-300">
            <Wrench className="h-6 w-6 text-teal-700" />
          </div>
          <div>
            <h1 className="text-3xl font-bold">Work Orders</h1>
            <p className="text-muted-foreground">
              View maintenance tasks for vehicles you operate
            </p>
          </div>
        </div>
      </div>
          ...
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {statusFilter !== "ALL"
                ...
                : "You don't have any assigned work orders for vehicles you operate"}
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          {workOrders.map((wo) => {
            ...
            return (
              <Card
                ...
                onClick={() => router.push(`/staff/work-orders/${wo.id}`)}
              >
                <CardContent className="py-4">
