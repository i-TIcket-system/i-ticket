...
interface WorkOrderChatProps {
  workOrderId: string
  workOrderNumber?: string // e.g., "WO-001234"
  className?: string
...
export function WorkOrderChat({
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const response = await fetch(`/api/company/work-orders/${workOrderId}/messages`)
  ...
  useEffect(() => {
    ...
    // Check if tab is visible using Visibility API
          ...
          >
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground">
                ...
                <p className="text-xs">Start the conversation about this work order</p>
              </div>
            ) : (
              messages.map((msg) => {
          ...
          <div className="p-4 border-t bg-background">
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Type a message about this work order..."
                disabled={sending}
                className="flex-1"
              />
