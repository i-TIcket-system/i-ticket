...
interface MechanicWorkOrderChatProps {
  workOrderId: string
...
export function MechanicWorkOrderChat({
  workOrderId,
  workOrderNumber,
  className,
  defaultExpanded = true,
}: MechanicWorkOrderChatProps) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      // Use mechanic API endpoint
      const response = await fetch(`/api/mechanic/work-orders/${workOrderId}/messages`)
          ...
          >
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground">
                ...
                <p className="text-xs">Communicate with admin and team about this work order</p>
              </div>
            ) : (
              messages.map((msg) => {
